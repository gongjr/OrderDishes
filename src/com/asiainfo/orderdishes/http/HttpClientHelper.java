package com.asiainfo.orderdishes.http;

import android.os.AsyncTask;
import android.util.Log;

import com.asiainfo.orderdishes.helper.ResponseInterface;
...
public class HttpClientHelper {
     ...
     * 测试环境
     */
    private String urladdress = "http://192.168.1.151:8080/buildsite/appController";
...
//		this.urladdress = "http://192.168.1.191:8080/Beautiful";
    ...
    /**
     * 单例线程池来管理网络访问的匿名线程 最大支持并发10条匿名线程，多余排队等待
     *
     * @return
     */
    public static synchronized ExecutorService getExecutor() {
        if (null == executorService) {
            executorService = Executors.newFixedThreadPool(10);// Executors的静态方法来创建，固定数量线程池
        }
    ...
    @SuppressWarnings("finally")
    public void ConnectWithAsyncTask(final String url,
...
//				Log.i("FragmentLife", "handler 111111111111111");
                String result = null;
                HttpPost httpPost = null;
                HttpGet httpGet = null;
                HttpClient client = getHttpClient();
                if (method.equals(HttpClientHelper.POST)) {
                    httpPost = new HttpPost(urladdress + url);
                    UrlEncodedFormEntity uefEntity = null;
                    try {
                        uefEntity = new UrlEncodedFormEntity(params, "UTF-8");
                    ...
                    Log.i(TAG, "URL_POST : " + urladdress + url);
                    ...
                    if (method.equals(HttpClientHelper.GET)) {
                        String address = urladdress + url;
                        Log.i(TAG, "URL_GET : " + address);
                        httpGet = new HttpGet(address);
                    } else {
                        Log.i(TAG, "URL_ADDRESS : " + url);
                        httpGet = new HttpGet(url);
                    }
                }
                HttpEntity entity = null;
                ...
                try {
                    ...
                    Log.i(TAG,
                            "SocketTimeoutException-----------------服务器响应超时，检查网络信号或服务器");
                    ...
                    Log.i(TAG, "result-----------------" + result);
                    Log.i(TAG,
                            "UnknownHostException-----------------请求主机地址无效，检查服务器");
                    // 请求的主机地址无效
                    e.printStackTrace();
                    result = "2";
                } catch (IOException e) {
                    Log.i(TAG, "result-----------------" + result);
                    Log.i(TAG,
                            "IOException-----------------建立连接异常，检查本地网络或确认地址是否有效");
...
//				Log.i("FragmentLife", "handler 22222222222222");
