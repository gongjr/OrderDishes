...
import com.asiainfo.orderdishes.util.UiUtils;
...
//        Log.i(TAG, "---a = " + a + "---w = " + w + "--leaf.x = " + leaf.x);
    ...
    private void initBitmap() {
        mLeafBitmap = ((BitmapDrawable) mResources.getDrawable(R.drawable.leaf2)).getBitmap();
    ...
    private class LeafFactory {
        ...
        // 生成一个叶子信息
        public Leaf generateLeaf(int index) {
            Leaf leaf = new Leaf();
            ...
            mLeafFloatTime = mLeafFloatTime <= 0 ? LEAF_FLOAT_TIME : mLeafFloatTime;
            if (index == 0) mAddTime = 0;
            else if (index == 1) mAddTime = 500;
            else mAddTime += random.nextInt((int) (mLeafFloatTime * 2));
